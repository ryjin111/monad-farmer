...
contract FarmingGame is Ownable, ReentrancyGuard, ERC721, ERC721URIStorage {
    ...
    constructor() ERC721("Monad Farming Achievements", "MFA") {
        ...
        cropSeedPrices[CropType.TOMATO] = 10;
        cropSeedPrices[CropType.CARROT] = 5;
        cropSeedPrices[CropType.POTATO] = 12;
        cropSeedPrices[CropType.CORN] = 15;
        cropSeedPrices[CropType.WHEAT] = 20;
        cropSeedPrices[CropType.STRAWBERRY] = 30;
        cropSeedPrices[CropType.GOLDEN_APPLE] = 200;
    }
    ...
    function _checkAchievements(address player) internal {
        ...
        // Golden Harvest (harvest legendary crop)
        if (!playerAchievements[player][AchievementType.GOLDEN_HARVEST] && p.cropCounts[CropType.GOLDEN_APPLE] >= 1) {
            _unlockAchievement(player, AchievementType.GOLDEN_HARVEST);
        }
        
        // Water Master (water 50 times - simplified to total planted for now)
        if (!playerAchievements[player][AchievementType.WATER_MASTER] && p.totalPlanted >= 50) {
            _unlockAchievement(player, AchievementType.WATER_MASTER);
        }
        
        // Crop Collector (harvest all crop types)
        uint256 uniqueCrops = 0;
        for (uint256 i = 0; i < 7; i++) {
            if (p.cropCounts[CropType(i)] > 0) {
                uniqueCrops++;
            }
        }
        if (!playerAchievements[player][AchievementType.CROP_COLLECTOR] && uniqueCrops >= 7) {
            _unlockAchievement(player, AchievementType.CROP_COLLECTOR);
        }
        ...
        // Speed Grower (harvest 10 crops in one day - simplified to total harvests for now)
        if (!playerAchievements[player][AchievementType.SPEED_GROWER] && p.totalHarvests >= 10) {
            _unlockAchievement(player, AchievementType.SPEED_GROWER);
        }
    }
    ...
    // Buy coins with MONAD - FIXED VERSION
    function buyCoins() external payable nonReentrant {
        require(msg.value > 0, "Must send MONAD to buy coins");
        
        // Convert msg.value from wei to ether first, then calculate coins
        uint256 monadAmount = msg.value / 1e18; // Convert wei to ether
        uint256 coinsToReceive = monadAmount * MONAD_TO_COINS_RATE;
        
        // Calculate fee (0.1%)
        uint256 fee = monadAmount * BUY_COINS_FEE / 1000;
        ...
        emit CoinsPurchased(msg.sender, monadAmount, coinsAfterFee);
    }
